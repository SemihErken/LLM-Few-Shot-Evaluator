    ...
    for task_name, task in task_dict.items():
        ...
        for key in task.instances[0].filtered_resps.keys():   
            ...
            for doc_id, doc in doc_iterator:
                ...
                if cnt > 24 : 
                  cevaplar = [req.filtered_resps[key] for req in requests]
                  max_deger = float('-inf')
                  maxindeks = None
                  for indeks, (deger,_) in enumerate(cevaplar):
                    if deger > max_deger:
                      max_deger = deger
                      max_indeks = indeks
                  ...
                  answer_prob = 0.0
                  if requests[0].doc["answerKey"] == "A": 
                    answer_prob = cevaplar[0]
                  elif requests[0].doc["answerKey"] == "B": 
                    answer_prob = cevaplar[1]
                  elif requests[0].doc["answerKey"]r == "C": 
                    answer_prob = cevaplar[2]
                  elif requests[0].doc["answerKey"] == "D": 
                    answer_prob = cevaplar[3]

                    print(requests[0].doc["Quesiton"])
                    print("Verilen cevap:",answer,"Doğru cevap:",requests[0].doc["Answer"])
                    print("OLASILIK")
                    print("Doğru cevap:",answer_prob,'  Üretilen cevap :',max_deger )
